...
public class ModelsController {
    ...
    private static Collection<? extends Item> findElements(Collection<String> elementsList) {
        ...
        Collection<? extends Item> elementsInModels = JPAEntityManagerUtils.executeQuery(Item.class, Model.GET_MODELS_USAGE_BY_ELEMENT_ID_LIST, params);
        result.addAll(elementsInModels);

        //find element directly on any layer
        Collection<? extends Item> elementsInLayers = JPAEntityManagerUtils.executeQuery(Item.class, Layer.GET_LAYERS_USAGE_BY_ELEMENT_ID_LIST, params);
        result.addAll(elementsInLayers);

        //find element directly on any system
        Collection<? extends Item> elementsInSystems = JPAEntityManagerUtils.executeQuery(Item.class, System.GET_SYSTEMS_USAGE_BY_ELEMENT_ID_LIST, params);
        result.addAll(elementsInSystems);

        //find element directly on any container
        Collection<? extends Item> elementsInContainers = JPAEntityManagerUtils.executeQuery(Item.class, Container.GET_CONTAINERS_USAGE_BY_ELEMENT_ID_LIST, params);
        result.addAll(elementsInContainers);

        //find element directly on any component
        Collection<? extends Item> elementsInComponents = JPAEntityManagerUtils.executeQuery(Item.class, Component.GET_COMPONENTS_USAGE_BY_ELEMENT_ID_LIST, params);
